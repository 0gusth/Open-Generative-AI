  ...
  return (
    <div className="group relative aspect-[4/5] rounded-xl cursor-pointer">
      <div
        ...
        className="absolute inset-0 rounded-xl overflow-hidden border border-white/5 bg-[#171719] transition-all group-hover:border-white/15 group-hover:scale-[1.02] shadow-2xl"
      >
        {agent.icon_url ? (
          <img
            src={agent.icon_url}
      ...
      {onEdit && (
        <button
          ...
          className="absolute top-3 right-3 w-8 h-8 rounded-full bg-black/60 border border-white/10 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-all hover:bg-white/15 hover:text-black hover:scale-110 z-10"
        >
...
function ConversationCard({ conv, onClick }) {
  ...
  return (
    <div
      ...
      className="group flex flex-col gap-3 bg-white/[0.03] border border-white/5 rounded-xl p-4 hover:border-white/15 hover:bg-white/5 transition-all cursor-pointer"
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[10px] font-black text-white/80 uppercase tracking-wider truncate">
    ...
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div
        className={`max-w-[75%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${
          isUser
            ? "bg-white/15 text-black font-medium"
...
export default function AgentStudio({ apiKey }) {
  ...
  if (view === "create") {
    return (
      <div className="h-full flex flex-col bg-[#0f0f10] text-white overflow-y-auto custom-scrollbar">
        <div className="flex-shrink-0 h-16 border-b border-white/5 flex items-center gap-6 px-8 bg-black/40">
          ...
          <h2 className="text-sm font-black uppercase tracking-[0.2em] text-white/80">Create Agent</h2>
        </div>
        ...
        <form onSubmit={handleCreateSubmit} className="max-w-2xl w-full mx-auto p-8 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-sm text-white focus:outline-none focus:border-white/15 transition-colors min-h-[140px] resize-none"
            />
          </div>
          ...
          <button
            ...
            className="w-full py-4 bg-white/15 text-black text-xs font-black uppercase tracking-[0.2em] rounded-xl hover:bg-white transition-all disabled:opacity-40 flex items-center justify-center gap-3"
          >
            {creating ? (
              <>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-8 space-y-4 max-w-3xl w-full mx-auto">
          {chatLoading ? (
            <div className="h-full flex items-center justify-center">
              <div className="w-10 h-10 border-2 border-white/5 border-t-white/70 rounded-full animate-spin" />
            </div>
          ) : (
            <>
              ...
              {sending && (
                <div className="flex justify-start">
                  <div className="bg-white/[0.04] border border-white/5 rounded-2xl px-4 py-3">
                    <div className="w-4 h-4 border-2 border-white/10 border-t-white/70 rounded-full animate-spin" />
                  </div>
                </div>
              )}
              {chatError && (
        ...
        <div className="flex-shrink-0 border-t border-white/5 p-6 bg-black/40">
          ...
          >
            <textarea
              ...
              rows={1}
              className="flex-1 bg-white/5 border border-white/10 rounded-xl p-3 text-sm text-white focus:outline-none focus:border-white/15 transition-colors resize-none max-h-40"
            />
            <button
              type="submit"
              disabled={!chatInput.trim() || sending}
              className="px-5 py-3 bg-white/15 text-black text-xs font-black uppercase tracking-widest rounded-xl hover:bg-white transition-all disabled:opacity-40"
            >
              Send
            </button>
          </form>
    ...
    <div className="h-full flex flex-col bg-[#0f0f10] text-white">
      ...
      <div className="flex-shrink-0 h-16 border-b border-white/5 flex items-center justify-between px-8 bg-black/40">
        <div className="flex items-center gap-8 h-full">
          <h2 className="text-sm font-black uppercase tracking-[0.2em] text-white/80">
            Agents
          </h2>
        ...
        <button
          ...
          className="px-6 py-2 bg-white/15 text-black text-[10px] font-black uppercase tracking-widest rounded-lg hover:bg-[#ebff66] transition-all active:scale-95 flex items-center gap-2"
        >
          ...
          conversations.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-white/10 gap-4">
              ...
              <button
                ...
                className="text-[10px] text-white/80 hover:text-white border border-white/15 hover:border-white/20 px-4 py-2 rounded-lg transition-colors"
              >
                Browse Templates
              </button>
            </div>
