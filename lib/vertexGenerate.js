...
export const isGoogleModel = (id) => Object.prototype.hasOwnProperty.call(VERTEX_MODELS, id);
export const vertexAvailable = () => googleCloudConfigured();

// Published Vertex rates (USD). Used for the cost chip; the authoritative
// number is always the GCP bill, and this is labelled as an estimate.
const IMAGE_COST = { '1k': 0.039, '2k': 0.134, '4k': 0.24 };
...
export async function vertexGenerateImage({ modelId, prompt, aspectRatio, tier = '1k', referenceImages = [], origin }) {
    const model = VERTEX_MODELS[modelId];
    if (!model) throw new Error(`${modelId} não tem equivalente no Vertex.`);
    ...
    return {
        ...
        cost: IMAGE_COST[String(tier).toLowerCase()] ?? IMAGE_COST['1k'],
        estimated: true,
        provider: 'vertex',
    };
}
...
export async function vertexGenerateVideo({ modelId, prompt, aspectRatio, duration = 5, image, origin, onPoll }) {
    const model = VERTEX_MODELS[modelId];
    if (!model) throw new Error(`${modelId} não tem equivalente no Vertex.`);
    const ai = await vertexClient();
