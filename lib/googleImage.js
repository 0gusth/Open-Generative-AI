...
//
// STATUS: built and tested, but DORMANT by design. The premise was a free
// image tier; a live probe of every Gemini image model returned "limit: 0"
// (no free image quota exists), and at 2K Google charges $0.101/image against
// Runware's $0.069. Kept because it is correct and one env var away from
// useful the day Google's pricing or quota changes.
//
...
// MEASURED Aug 2026 with a live key: every Gemini image model answers
// "limit: 0" on the free tier — there is NO free image quota. And at the 2K
// tier this app uses, Google charges $0.101/image against Runware's $0.069.
// So image routing is OFF unless explicitly switched on: having a key for
// TEXT (which IS free) must never silently make images more expensive.
const IMAGE_ROUTING_ENABLED = process.env.GOOGLE_IMAGE_ENABLED === '1';

export const googleImageConfigured = () =>
    IMAGE_ROUTING_ENABLED && (FREE_KEYS.length > 0 || !!PAID_KEY);
