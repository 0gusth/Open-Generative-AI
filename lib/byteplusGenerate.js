...
export async function byteplusGenerateImage({
    ...
    const { size, pixels, cappedFrom } = byteplusSize(model, aspectRatio, tier);
    const body = {
        model,
        prompt: prompt || '',
        size,
    ...
    return {
        ...
        // Set when the model cannot reach the tier that was asked for (Pro
        // tops out around 2.1K). The card says so — a paid render coming back
        // smaller than requested is never allowed to pass unmentioned.
        cappedFrom: cappedFrom || null,
