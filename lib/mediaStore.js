...
function fileName(mimeType) {
    const ext = EXT_BY_MIME[mimeType] || (String(mimeType).startsWith('video/') ? 'mp4' : 'png');
    const stamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    return `gen-${stamp}-${Math.random().toString(36).slice(2, 8)}.${ext}`;
}

// Store base64 media (image or video), return a public URL.
export async function storeMedia(base64, mimeType = 'image/png') {
...
// Kept so existing callers keep reading naturally.
export const storeImage = storeMedia;
